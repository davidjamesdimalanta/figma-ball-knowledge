...
Use the the discovery audit script to discover all local collections and styles. Do not use `get_variable_defs` for discovery — it only returns tokens already bound to a specific node.

**Local variables only — `getLocalVariableCollectionsAsync()` is not enough on its own.** Files using a shared or published library store their tokens remotely. the remote library discovery call (`search_design_system("", { includeVariables: true })`) surfaces those. Always cross-reference both before declaring a token missing.
...
- Using `get_variable_defs` to discover available tokens — use the the discovery audit script.
