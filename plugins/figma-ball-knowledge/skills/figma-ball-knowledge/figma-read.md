...
Always complete the discovery audit in `SKILL.md` before using any tool in this file.
...
| `use_figma` (audit) | Local variable collections and styles — the full discovery script from SKILL.md (Discovery Audit). |
...
- Using `get_variable_defs` to discover what tokens are *available* — it only returns what is already bound. Use the the discovery audit script for discovery.
