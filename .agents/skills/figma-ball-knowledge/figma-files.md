...
Resolve page targets by ID — never by name. Page names are not unique and can be renamed. The node-id from a Figma URL is the authoritative target reference (see the URL Resolution section in `SKILL.md`).
