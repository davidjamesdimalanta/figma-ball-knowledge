...
6. If the failure revealed undocumented Figma API behavior — a method name, a node-type restriction, an async semantic no capability file mentions — tell the user and propose updating the relevant capability file so the next session doesn't rediscover it.
