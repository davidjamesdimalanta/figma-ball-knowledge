...
**Source:** the discovery audit results plus targeted follow-up searches. Do not rely on a single broad `search_design_system("")` call — it returns noisy results and misses components whose names don't surface well in a general query. Run multiple short, specific searches for each unresolved element:
