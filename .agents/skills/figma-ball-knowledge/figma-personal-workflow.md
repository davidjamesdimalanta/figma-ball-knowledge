...
- The parallel discovery script in `figma-ball-knowledge`
...
At the start of each session, `figma-ball-knowledge` loads the relevant preferences into working memory. This means:
...
| Global defaults applying across all tasks | `figma-ball-knowledge` |
...
3. Move global preferences (applying across all taskflows) to `figma-ball-knowledge` and remove the per-skill duplicate.
